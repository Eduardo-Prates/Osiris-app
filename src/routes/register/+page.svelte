<script lang="ts">
	...
	import { Eye, EyeOff, User, Phone, Mail, Lock } from 'lucide-svelte';
...
<div class="flex min-h-screen flex-col bg-background px-6 py-12">
	...
	<form onsubmit={(e) => { e.preventDefault(); handleSignup(); }} class="flex flex-col gap-4">
		<div class="relative">
			<User class="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
			<input
				type="text"
				bind:value={nome}
		...
		<div class="relative">
			<Phone class="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
			<input
				type="tel"
				bind:value={telefone}
		...
		<label class="flex items-start gap-2 text-sm">
			...
			<span class="text-muted-foreground">
				Eu li e concordo com os <a href="/termos" class="text-primary hover:underline">Termos e Condições</a>
			</span>
		</label>
	...
	<!-- Social signup -->
	<button class="flex w-full items-center justify-center gap-2 rounded-lg border border-input bg-background py-3 transition-colors hover:bg-muted">
		...
		<span class="font-medium text-foreground">Realize login</span>
	</button>

	<!-- Login link -->
	<p class="mt-6 text-center text-sm text-muted-foreground">
		Já possui uma conta? <a href="/login" class="font-medium text-primary hover:underline">Realize login</a>
	</p>
</div>
