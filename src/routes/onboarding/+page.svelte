  ...
  const slides = [
    {
      ...
      description:
        "Pesquise e encontre tudo que você precisa para compras, aluguéis ou serviços, diretamente em seu celular.",
      image: carFallback,
    },
    {
      title: "Negocie e converse com o anunciante",
      description:
        "Fale com os vendedores de cada produto, tire suas dúvidas e envie mensagens em tempo real para uma negociação.",
      image: chatFallback,
    },
    {
      title: "Gerencie seus anúncios e perfil",
      description:
        "Cadastre produtos e serviços, acompanhe suas vendas, edite seu anúncio e receba notificações de novos negócios.",
...
<div class="flex min-h-screen flex-col bg-surface-50">
  ...
  <div class="flex items-center justify-between p-4">
    <span class="text-sm text-surface-600">{currentSlide + 1}/3</span>
    <button
  ...
  <div class="flex flex-col items-center gap-6 p-6">
    <div class="flex gap-2">
      {#each slides as _, index}
        <button
          ...
          class="h-2 rounded-full transition-all {currentSlide === index
            ? 'w-8 bg-primary-500'
            : 'w-2 bg-surface-400'}"
    ...
    <button
      on:click={nextSlide}
      class="btn variant-filled-primary w-full rounded-lg px-6 py-3 font-semibold"
    >
