...
<div class="flex min-h-screen flex-col bg-background px-6 py-12">
	...
	<!-- Welcome text -->
	<h1 class="mb-2 text-3xl font-bold text-foreground">Bem-vindo de volta!</h1>
	<p class="mb-8 text-muted-foreground">Entre com sua conta para continuar</p>
	...
	<form onsubmit={(e) => { e.preventDefault(); handleLogin(); }} class="flex flex-col gap-4">
		...
		<button type="button" class="self-start text-sm text-primary hover:underline">
			Esqueceu a senha?
		</button>
	...
	<button class="flex w-full items-center justify-center gap-2 rounded-lg border border-input bg-background py-3 transition-colors hover:bg-muted">
		...
		<span class="font-medium text-foreground">Cadastre-se</span>
	</button>

	<!-- Sign up link -->
	<p class="mt-6 text-center text-sm text-muted-foreground">
		Não tem uma conta? <a href="/cadastro" class="font-medium text-primary hover:underline">Cadastre-se</a>
	</p>
</div>
